...
5. ~~Copies through the trusted-source path.~~ Shipped: the same pipe as the DigitalOcean path, as the clone's owner, since Cloud SQL grants every created user cloudsqlsuperuser. The trusted source must be an authorized network on the instance or run the Auth Proxy, and the report names the `gcloud sql instances patch --authorized-networks` line when it cannot connect. Still queued: the `/cloudsql/` socket and the declaration's Cloud SQL volume, and custom domain mappings.
