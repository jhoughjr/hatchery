...
final class HealthRequestTests: XCTestCase {
    ...
    func testAppPlatformServiceUsesItsPublicName() throws {
        ...
        XCTAssertEqual(probe.url.absoluteString, "https://tenant.example.com/health")
        XCTAssertNil(probe.hostHeader)
    }
    ...
    /// `/health` is the estate convention, generated by `microservice-auth`. gsx-gateway is the
    /// one exception at `/healthz`, so a single path for every kind would probe the wrong route.
    func testHealthPathFollowsTheServiceKind() {
        XCTAssertEqual(ServiceKind.mwserver.defaultHealthPath, "/health")
        XCTAssertEqual(ServiceKind.gsxGateway.defaultHealthPath, "/healthz")
